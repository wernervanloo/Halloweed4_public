...
int screenbit  = 0x10000;
int bitmapbit  = 0x20000;
int spritebit  = 0x40000;
int pointerbit = 0x80000;
...
{
	ofstream out2;
	out2.open("../bin/sprite_koppel.asm", ios::out | ios::binary);
	out2 << ".var ht_sprites = Hashtable()" << char(0x0D);

	for (int filectr = 0x0400; filectr < (65536 - 2); filectr++) 
	{ 
		if (((memory2copy[filectr]) & spritebit) == spritebit)
		{
			out2 << ".eval ht_sprites.put(" << filectr << "," << ((memory2copy[filectr])&0xffff) << ")" << char(0x0D);
		}
	}
	out2.close();

	out2.open("../bin/screen_koppel.asm", ios::out | ios::binary);
	out2 << ".var ht_screen = Hashtable()" << char(0x0D);
	for (int filectr = 0x0400; filectr < (65536 - 2); filectr++)
	{
		if (((memory2copy[filectr]) & screenbit) == screenbit)
		{
			out2 << ".eval ht_screen.put(" << filectr << "," << ((memory2copy[filectr]) & 0xffff) << ")" << char(0x0D);
		}
	}
	out2.close();

	out2.open("../bin/bitmap_koppel.asm", ios::out | ios::binary);
	out2 << ".var ht_bitmapdata = Hashtable()" << char(0x0D);
	for (int filectr = 0x0400; filectr < (65536 - 2); filectr++)
	{
		if (((memory2copy[filectr]) & bitmapbit) == bitmapbit)
		{
			out2 << ".eval ht_bitmapdata.put(" << filectr << "," << ((memory2copy[filectr]) & 0xffff) << ")" << char(0x0D);
		}
	}
	out2.close();

	out2.open("../bin/pointer_koppel.asm", ios::out | ios::binary);
	out2 << ".var ht_pointerdata = Hashtable()" << char(0x0D);
	for (int filectr = 0x0400; filectr < (65536 - 2); filectr++)
	{
		if (((memory2copy[filectr]) & pointerbit) == pointerbit)
		{
			out2 << ".eval ht_pointerdata.put(" << filectr << "," << ((memory2copy[filectr]) & 0xffff) << ")" << char(0x0D);
		}
	}
	out2.close();
	return;
		...
		if ((onlywritemax == false || line == findlines)) {
				...
				if (memorycopy[screen * 0x400 + 0x3fa] == 'p') {
					...
					for (sprite = 0; sprite < sprites; sprite++) {
						for (spriteimage = 0; (spriteimage < 256 && occupied <= sprite); spriteimage++) {
							...
							if (!imageused) {
								memory2copy[screen * 0x400 + 0x3fa + sprite] = spriteimage | pointerbit;
								occupied++;
								for (spritebyte = 0; spritebyte < 64; spritebyte++) {
			...
			for (spriteline = 0; spriteline < line; spriteline++) {
				for (sprite = 0; sprite < sprites; sprite++) {
					...
					spritepointer = 0xff & (memory2copy[(d018tab[spriteline] & 0xf0) * 0x040 + 0x4000 * (((dd00tab[spriteline] ^ 3) & (dd02tab[spriteline])) & 3) + 0x3fa + sprite]);
...
void convertToSprites()
{
	for (int x = 0; x < (6 * 3); x++)
	{
		for (int y = 0; y < 166; y++)
		{
			...
			spritedata[y * 6 * 3 + x] = spritebit | 6 * 3 * y + x;
		}
	}
	return;
}
...
int main(int argc, char *argv[])
{
	...
	// read test picture
	int length;
	flifile = ReadAllBytes("../bin/convert_AFLI.prg", &length);
	...
	for (int i = 0; i < 8192; i++) flidata[screenoffset + i] = screenbit + i;
	for (int i = 0; i < 8192; i++) flidata[bitmapoffset + i] = bitmapbit + i;
